...
function sanitizeApartment(input: unknown): ApartmentRow {
  ...
  return {
    ...
    north_boundary: trimOrNull(r.north_boundary, 255),
    south_boundary: trimOrNull(r.south_boundary, 255),
    east_boundary: trimOrNull(r.east_boundary, 255),
    west_boundary: trimOrNull(r.west_boundary, 255),
    qr_code: trimText(r.qr_code, 255),
    qr_access_token: trimOrNull(r.qr_access_token, 255),
    qr_access_status: trimOrNull(r.qr_access_status, 50),
...
function toApartmentApiPayload(row: ApartmentRow): Obj {
  return {
    ...
    north_boundary: row.north_boundary ?? null,
    south_boundary: row.south_boundary ?? null,
    east_boundary: row.east_boundary ?? null,
    west_boundary: row.west_boundary ?? null,
    qr_code: row.qr_code,
